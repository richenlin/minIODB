# 基于MinIO、DuckDB和Redis的分布式OLAP系统设计方案
...
*   **轻量化与高性能**: 所有组件（MinIO, DuckDB, Redis）都是以高性能和低资源占用著称的。DuckDB尤其擅长直接查询Parquet等列式存储格式，无需数据导入，极大地提升了查询效率。
...
backup:
  ...
  minio:
    endpoint: "backup-minio.example.com:9000"
    access_key_id: "backup_user"
    secret_access_key: "backup_password"
    use_ssl: true
    bucket: "olap-backup-bucket"
``` 