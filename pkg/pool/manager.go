...
func NewPoolManager(config *PoolManagerConfig, logger *zap.Logger) (*PoolManager, error) {
	if config == nil {
	...
	// 创建MinIO连接池
	minioPool, err := NewMinIOPool(config.MinIO, logger)
	if err != nil {
		if redisPool != nil {
			redisPool.Close()
		}
	...
	if config.BackupMinIO != nil {
		backupPool, err = NewMinIOPool(config.BackupMinIO, logger)
		if err != nil {
			logger.Sugar().Infof("WARN: failed to create backup MinIO pool: %v", err)
			// 备份连接池失败不影响主要功能
		}
	}
...
func (pm *PoolManager) healthCheckLoop() {
	...
	for {
		...
		case <-pm.stopHealthCheck:
			pm.logger.Info("Health check stopped")
			return
		}
	}
}
...
func (pm *PoolManager) performHealthCheck() {
	...
	if pm.backupPool != nil {
		if err := pm.backupPool.HealthCheck(ctx); err != nil {
			pm.logger.Sugar().Infof("WARN: Backup MinIO health check failed: %v", err)
		}
	}
}
...
func (pm *PoolManager) UpdateRedisPool(config *RedisPoolConfig) error {
	...
	// 创建新的Redis连接池
	newPool, err := NewRedisPool(config, pm.logger)
	if err != nil {
	...
	pm.logger.Info("Redis connection pool updated successfully")
	return nil
}
...
func (pm *PoolManager) UpdateMinIOPool(config *MinIOPoolConfig) error {
	...
	// 创建新的MinIO连接池
	newPool, err := NewMinIOPool(config, pm.logger)
	if err != nil {
	...
	pm.logger.Info("MinIO connection pool updated successfully")
	return nil
}
...
func (pm *PoolManager) UpdateBackupMinIOPool(config *MinIOPoolConfig) error {
	...
	if config == nil {
		...
		pm.logger.Info("Backup MinIO connection pool disabled")
		return nil
	}
	...
	pm.logger.Info("Backup MinIO connection pool updated successfully")
	return nil
}
...
func (pm *PoolManager) Close() error {
	...
	pm.logger.Info("All connection pools closed successfully")
	return nil
}
