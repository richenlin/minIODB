...
func NewMinIOPool(ctx context.Context, config *MinIOPoolConfig) (*MinIOPool, error) {
	if config == nil {
	...
	logger.LogInfo(ctx, "Creating MinIO client for endpoint: %s", zap.String("endpoint", config.Endpoint))
	...
	logger.LogInfo(ctx, "Testing MinIO connection to %s with timeout %v", zap.String("endpoint", config.Endpoint), zap.Duration("timeout", config.RequestTimeout))
...
func (p *MinIOPool) healthCheck(ctx context.Context) error {
	...
	if err != nil {
		logger.LogInfo(ctx, "MinIO health check failed: %v (endpoint: %s)", zap.Error(err), zap.String("endpoint", p.config.Endpoint))
		return err
	}
	logger.LogInfo(ctx, "MinIO health check successful, found %d buckets (endpoint: %s)", zap.Int("buckets", len(buckets)), zap.String("endpoint", p.config.Endpoint))
	return nil
}
...
// Close 关闭连接池
func (p *MinIOPool) Close(ctx context.Context) {
	if p.transport != nil {
...
func (p *MinIOPool) UpdateConfig(ctx context.Context, newConfig *MinIOPoolConfig) error {
	p.mutex.Lock()
	...
	// 创建新的连接池
	newPool, err := NewMinIOPool(ctx, newConfig)
	if err != nil {
		return err
	}
