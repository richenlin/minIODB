...
func NewTableManager(redisClient *redis.Client, primaryMinio, backupMinio storage.Uploader, cfg *config.Config) *TableManager {
	return &TableManager{
...
func (tm *TableManager) CreateTable(ctx context.Context, tableName string, tableConfig *config.TableConfig, ifNotExists bool) error {
	// 验证表名
	if !tm.config.IsValidTableName(tableName) {
	...
	if totalTables >= tm.config.TableManagement.MaxTables {
		return fmt.Errorf("maximum number of tables (%d) reached", tm.config.TableManagement.MaxTables)
	}
	
	// 使用默认配置填充未设置的字段
	if tableConfig == nil {
		tableConfig = &tm.config.Tables.DefaultConfig
	} else {
		// 合并配置
		defaultConfig := tm.config.Tables.DefaultConfig
	...
	// 添加到表列表
	if err := tm.redisClient.SAdd(ctx, "tables:list", tableName).Err(); err != nil {
	...
	if err := tm.redisClient.HMSet(ctx, configKey, configData).Err(); err != nil {
		return fmt.Errorf("failed to set table config: %w", err)
	}
	
	// 设置创建时间
	createdAtKey := fmt.Sprintf("table:%s:created_at", tableName)
	if err := tm.redisClient.Set(ctx, createdAtKey, now, 0).Err(); err != nil {
	...
	if err := tm.redisClient.HMSet(ctx, statsKey, statsData).Err(); err != nil {
...
func (tm *TableManager) ListTables(ctx context.Context, pattern string) ([]*TableManagerInfo, error) {
	// 获取所有表名
	tableNames, err := tm.redisClient.SMembers(ctx, "tables:list").Result()
	if err != nil {
...
func (tm *TableManager) TableExists(ctx context.Context, tableName string) (bool, error) {
	return tm.redisClient.SIsMember(ctx, "tables:list", tableName).Result()
}
...
func (tm *TableManager) EnsureTableExists(ctx context.Context, tableName string) error {
	...
	if !exists {
		if !tm.config.TableManagement.AutoCreateTables {
...
func (tm *TableManager) UpdateLastWrite(ctx context.Context, tableName string) error {
	lastWriteKey := fmt.Sprintf("table:%s:last_write", tableName)
	now := time.Now().UTC().Format(time.RFC3339)
	return tm.redisClient.Set(ctx, lastWriteKey, now, 0).Err()
}
...
func (tm *TableManager) getTableInfo(ctx context.Context, tableName string) (*TableManagerInfo, error) {
	...
	// 获取创建时间
	createdAtKey := fmt.Sprintf("table:%s:created_at", tableName)
	createdAt, err := tm.redisClient.Get(ctx, createdAtKey).Result()
	...
	// 获取最后写入时间
	lastWriteKey := fmt.Sprintf("table:%s:last_write", tableName)
	lastWrite, err := tm.redisClient.Get(ctx, lastWriteKey).Result()
	if err != nil && err != redis.Nil {
		lastWrite = ""
	}
...
func (tm *TableManager) getTableConfigFromRedis(ctx context.Context, tableName string) (*config.TableConfig, error) {
	configKey := fmt.Sprintf("table:%s:config", tableName)
	configData, err := tm.redisClient.HGetAll(ctx, configKey).Result()
	if err != nil {
	...
	if len(configData) == 0 {
		// 如果没有配置，返回默认配置
		return &tm.config.Tables.DefaultConfig, nil
	}
...
func (tm *TableManager) getTableStats(ctx context.Context, tableName string) (*TableManagerStats, error) {
	statsKey := fmt.Sprintf("table:%s:stats", tableName)
	statsData, err := tm.redisClient.HGetAll(ctx, statsKey).Result()
	if err != nil {
...
func (tm *TableManager) deleteTableData(ctx context.Context, tableName string) (int, error) {
	...
	for _, key := range keys {
		files, err := tm.redisClient.SMembers(ctx, key).Result()
		if err != nil {
		...
		for _, file := range files {
			if err := tm.primaryMinio.RemoveObject(ctx, tm.config.MinIO.Bucket, file, minio.RemoveObjectOptions{}); err != nil {
			...
			// 从备份MinIO删除文件（如果存在）
			if tm.backupMinio != nil && tm.config.Backup.Enabled {
				if err := tm.backupMinio.RemoveObject(ctx, tm.config.Backup.MinIO.Bucket, file, minio.RemoveObjectOptions{}); err != nil {
		...
		// 删除索引键
		if err := tm.redisClient.Del(ctx, key).Err(); err != nil {
...
func (tm *TableManager) getTotalTableCount(ctx context.Context) (int, error) {
	count, err := tm.redisClient.SCard(ctx, "tables:list").Result()
	if err != nil {
		return 0, err
	}
