...
func (h *RecoveryHandler) Recover() {
	if r := recover(); r != nil {
		...
		if h.logger != nil {
			h.logger.Error("PANIC", zap.String("panic_msg", panicMsg))
		}
...
func (hc *HealthChecker) logResults(results []HealthStatus) {
	...
	for _, result := range results {
		if result.Status != "healthy" {
			hc.logger.Error("Health check failed", zap.String("name", result.Name), zap.String("error", result.Error), zap.Duration("duration", result.Duration))
		}
	}
}
...
func (ar *AutoRecovery) TryRecover(ctx context.Context, err error) error {
	...
	for name, strategy := range ar.strategies {
		if strategy.CanRecover(err) {
			if ar.logger != nil {
				ar.logger.Error("Attempting recovery with strategy", zap.String("name", name))
			}

			if recoveryErr := strategy.Recover(ctx, err); recoveryErr == nil {
				if ar.logger != nil {
					ar.logger.Error("Recovery successful with strategy", zap.String("name", name))
				}
				return nil
			}
		}
