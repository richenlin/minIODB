...
- **MinIO双池故障切换**: 主存储池+备份存储池，主池故障自动切换到备份池，主池恢复后自动切回，异步数据同步保证最终一致性
- **智能健康检查**: 15秒间隔健康检查，故障快速发现和自动恢复，故障状态Redis持久化
...
*   **MinIO连接池管理器**:
    1.  **双池架构**: 维护主MinIO池和备份MinIO池，主池负责所有读写，备份池负责异步数据备份。
    2.  **自动故障切换**: 主池故障时自动切换到备份池提供临时服务，主池恢复后自动切回。
    3.  **异步数据同步**: 主池写入成功后，异步将数据同步到备份池（队列大小1000，3个worker并发），不阻塞主流程。
    4.  **智能健康检查**: 15秒间隔检查主池和备份池健康状态，故障快速发现。
    5.  **状态持久化**: 故障切换状态存储在Redis，分布式节点共享，保证一致性。
    6.  **连接优化**: 优化S3连接参数，提升上传下载性能。
    7.  **性能监控**: Prometheus指标监控切换次数、同步队列深度、成功/失败率。
*   **统一管理接口**:
...
系统支持两种部署模式，通过配置文件中的`redis.enabled`开关进行控制：
